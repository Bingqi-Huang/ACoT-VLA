...
### 9. Full offline-cache fast path for the full generalist run

If the bottleneck is still raw parquet + HEVC video decode on the A100 machine, use the newer offline-cache path.

This path is also additive:

- raw path is still `scripts/train.py`
- old fast path is still `scripts/train_fast.py`
- offline cache path is `scripts/build_offline_cache.py` + `scripts/train_offline_cache.py`

Recommended end-to-end workflow:

#### Step A. Prepare the source machine that will build the cache

This is intended for the faster U9 machine, not the slower A100 storage machine.

Use the same dataset root and assets as the normal training path:
...
If needed, clean the metadata once:

```bash
uv run ./scripts/cleanup_depth_metadata.py "$ACOT_CHALLENGE_DATA_ROOT"
```

#### Step B. Compute norm stats first

The offline cache stores the final transformed sample payload, so it must use the same norm stats as the real training run.
...
#### Step C. Build the offline shard cache on the U9 machine

This step applies the legacy transform chain offline and writes per-sample shard arrays.

```bash
uv run python scripts/build_offline_cache.py \
    --base-config-name acot_challenge_generalist_lora_generalist \
    --cache-root /mnt/fast-disk/acot_generalist_offline_cache \
    --split all \
    --shard-size 2048 \
    --num-workers 8
```
...
- resolves the same episode split manifest as the legacy path
- resolves the same `subtask` valid indices as the legacy path
- iterates valid indices in ascending order for cache build locality
- applies:
  - repack transforms
  - data transforms
  - Normalize
  - model transforms
- writes the final sample payload into `.npy` shards plus a cache manifest
...
#### Step D. Copy the cache to the A100 machine

Copy the whole cache directory as-is. The manifest is path-agnostic, so it should survive the machine change.

Example destination:

```bash
/ssd_workspace/acot_generalist_offline_cache
```

#### Step E. Verify the cache on the A100 machine before training
...
uv run python scripts/verify_offline_cache.py \
    --base-config-name acot_challenge_generalist_lora_generalist \
    --cache-root /ssd_workspace/acot_generalist_offline_cache \
    --split all
```

This checks:

- random raw sample vs cached sample equality
- small-batch raw vs cached equality
- manifest compatibility with the current config, split, and norm stats

#### Step F. Run a short debug training job from cache

```bash
DEBUG_MODE=true uv run python scripts/train_offline_cache.py \
    --base-config-name acot_challenge_generalist_lora_generalist \
    --cache-root /ssd_workspace/acot_generalist_offline_cache \
    --exp-name generalist_offline_cache_debug \
    --overwrite
```

Check:

- first batch initializes successfully
- training loss logs appear
- validation loader initializes
- checkpoint writing works
- `train_metrics.jsonl` is written in the checkpoint root

#### Step G. Run the real offline-cache training job
...
#### Step H. Evaluate the trained checkpoint

The checkpoint format is intended to remain compatible with the existing tooling.

Existing offline eval path:
...
#### Step I. Compatibility expectations

The offline-cache path is meant to preserve:
...
- checkpoint `assets`
- `train_metrics.jsonl`
- downstream serving compatibility

So the produced checkpoint should still be usable by the existing checkpoint-serving path.

#### Step J. Important caveat

The raw ACOT transform path contains prompt injection randomness.

For the offline-cache path, this prompt injection is made deterministic per sample index during cache generation so that:

- cache generation is reproducible
- cache verification can compare raw vs cached samples exactly

This is the main current semantic difference between the raw path and the offline-cache path.
