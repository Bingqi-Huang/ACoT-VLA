...
def _evaluate_checkpoint(
    ...
    *,
    ...
    if not hasattr(model, "compute_loss_per_example") or not hasattr(model, "teacher_force_actions"):
        raise ValueError(f"Model type {type(model).__name__} does not implement offline evaluation hooks.")

    eval_config = dataclasses.replace(train_config, batch_size=batch_size, num_workers=num_workers)
    ...
    try:
        for batch_index, batch in enumerate(val_loader):
            ...
            batch_size_current = np.asarray(batch[1]).shape[0]
            task_names = (
                np.asarray(val_loader.last_metadata()["task"])  # type: ignore[union-attr]
                if getattr(val_loader, "last_metadata", None) is not None and val_loader.last_metadata() is not None
                else np.full(batch_size_current, "unknown")
            )
            observation = batch[0]
            actions = batch[1]
...
def main(args: Args) -> None:
    ...
    num_workers = train_config.num_workers if args.num_workers is None else args.num_workers
    summary_rows = []
