...
def _reasoning2action_repo_ids(*dataset_names: str) -> list[str]:
    return [os.path.join(_REASONING2ACTION_DATA_ROOT, dataset_name) for dataset_name in dataset_names]


def _reasoning2action_repack_transforms() -> _transforms.Group:
    return _transforms.Group(
        inputs=[
            _transforms.RepackTransform(
                {
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        val_num_batches=32 if not os.getenv("DEBUG_MODE", default=False) == "true" else 2,
        num_workers=24 if not os.getenv("DEBUG_MODE", default=False) == "true" else 1,
        batch_size=288 if not os.getenv("DEBUG_MODE", default=False) == "true" else 16,
        grad_accum_steps=1,
    ...
    # Continued training with weak-task upweighting by dataset oversampling.
    # We upweight low-score or easy-to-forget tasks by repeating their dataset shards
    # in repo_id while keeping sampler="subtask".
    TrainConfig(
        name="acot_challenge_generalist_continued_reweighted",
        model=acot_vla.ACOTConfig(
            coarse_action_horizon=30,
            action_horizon=30,
            paligemma_variant="gemma_2b_lora",
        ...
        data=dataclasses.replace(
            _reasoning2action_data_config(
                _reasoning2action_repo_ids(
                    ...
                    "clean_the_desktop_part_1",
                    "clean_the_desktop_part_2",
                    # Reweight weak/priority tasks: sorting (very low), desktop cleanup,
                    # place-block, shelf-straighten, pour-workpiece, and open-door.
                    "sorting_packages_part_1",
                    "sorting_packages_part_2",
                    "sorting_packages_part_3",
                    "sorting_packages_part_1",
                ...
                asset_id=os.getenv("ACOT_CHALLENGE_GENERALIST_ASSET_ID", "reasoning2action_sim_generalist"),
                split_name="acot_challenge_generalist_continued_reweighted",
            ),
        ),
        lr_schedule=_optimizer.CosineDecaySchedule(
            warmup_steps=1_000,
        ...
        save_interval=1000 if not os.getenv("DEBUG_MODE", default=False) == "true" else 100,
        val_interval=1000 if not os.getenv("DEBUG_MODE", default=False) == "true" else 50,
        val_num_batches=64 if not os.getenv("DEBUG_MODE", default=False) == "true" else 2,
        num_workers=24 if not os.getenv("DEBUG_MODE", default=False) == "true" else 1,
